...
from sheet_db import log_hearing_traffic, log_lecture_status
...
class mainapp:
    def __init__(self):
        ...
        self.process_status = {'go_ahead':False}
    ...
    def home(self):
        ...
        with st.expander("Lecture status",expanded=True):
            userdata4L = self.userdata4L
            status_banner = st.empty()
            # st.write(userdata4L)
            
            lecture_status_4user = json.loads(userdata4L['status']) if len(userdata4L.keys())>1 else {'status':'pending'}
            _is_new_entry = lecture_status_4user['status']=='pending'
            _is_midway = lecture_status_4user['status']=='in progress'
            _is_completed = lecture_status_4user['status']=='completed'
            
            if lecture_status_4user['status']=='in progress':
                with status_banner.container():
                    st.markdown(f"### :orange[You have heard this lecture until] :gray[{lecture_status_4user['heard_until']} mins]")
                    st.caption(f"last modified on `{userdata4L['last_modified']}`")
            
            elif lecture_status_4user['status']=='completed':
                with status_banner.container():
                    st.markdown(f"### :green[You have completed this lecture]")
                    st.caption(f"last modified on `{userdata4L['last_modified']}`")
            
            elif lecture_status_4user['status']=='pending':
                with status_banner.container():
                    st.markdown(f"### :red[You have not started this lecture]")
            
            user_status = st.pills(":gray[update status as]",
                                options=['in progress','completed'],
                                selection_mode='single',
                                default=None if _is_new_entry else lecture_status_4user['status'],
                                disabled=_is_completed)
            
            if user_status=='in progress':
                heard_until = st.number_input(label="completed until (in minutes)",
                ...
                if len(userdata4L.get('summary',""))>0:
                    ...
                    if view_mode=='edit':
                        lecture_summary = st.text_area(label="Save Striking points so far",
                                                height=100,
                                                max_chars=1500,
                                                value="" if _is_new_entry else userdata4L['summary']).strip()
                    else:
                        st.markdown(userdata4L['summary'])
                        lecture_summary = ""
                    ...
                    lecture_summary = st.text_area(label="Save Striking points so far",
                                               ...
                                               max_chars=1500,
                                               value="" if _is_new_entry else userdata4L['summary']).strip()
                if (len(lecture_summary) < len(userdata4L.get('summary',''))) and view_mode=='edit':
                    st.warning("Your older notes was more detailed")
                
                if heard_until and heard_until>0:
                    st.button("Update status as `in progress`",
                              on_click=lambda : self.process_status.update({'go_ahead':True}))
                    
                    if self.process_status['go_ahead']:
                        self.process_status.update({'go_ahead':False})
                        with st.spinner("updating"):
                            response = log_lecture_status({
                                "status_type": 'new_entry' if _is_new_entry else 'update_entry',
                                'db_row': None if _is_new_entry else userdata4L['row_id'],
                                
                                "lecture_id": self.lecture_info['id'],
                                "user_id": self.user_info['id'],
                                'status':{'status':"in progress",
                                           'heard_until': heard_until,
                                            },
                                'notes': lecture_summary,
                            })
                            self.userdata4L = response
                            st.rerun()
                        st.success("updated successfuly")
                            
                else:
                    st.button("Update status as in progress",disabled=True)
                ...
                if lecture_status_4user['status'] in  ['in progress','pending']:
                    st.button("Mark as completed",
                            on_click=lambda : self.process_status.update({'go_ahead':True}))
                
                elif lecture_status_4user['status']=='completed':
                    st.button("Update notes",
                            on_click=lambda : self.process_status.update({'go_ahead':True}))
                if (len(lecture_summary) < len(userdata4L.get('summary',''))) and view_mode=='edit':
                    st.warning("Your older notes was more detailed")
        ...
        if server =='ytube':
            height = st.slider("video height",min_value=400,max_value=800,value=500)
            
            html_code = f"""
            <iframe
            src="https://www.youtube.com/embed/{lec_id}?autoplay=0&mute=1&controls=1"
            ...
            """
            myHTML(html=html_code,height=height)
        else:
