    ...
    if not file_exists:
        ...
        if server == 'mega':
            ...
            with st.spinner("downloading from google drive..."):
                ...
                gdown.download(
                    url, f"{root_dir}/{filename}", quiet=False, fuzzy=True)
                msgbox.success("...done")
            msgbox.empty()

    else:
        # change the filename so that it points to actual file
        existing_file = [i for i in available_file_raw if i.split("^")[
            1] == filename]
...
def install_ffmpeg():
    try:
        ...
        try:
            ...
            # You can change `apt` to `dnf` or `pacman` for other distributions
